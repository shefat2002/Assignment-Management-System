...
export default function DashboardLayout({ children, allowedRole }: DashboardLayoutProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const userEmail = decoded['http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress'] || decoded.email || `${currentRole?.toLowerCase()}@system.com`;
      const userName = decoded['http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name'] || decoded.name || decoded.unique_name || currentRole || 'User';
      setEmail(userEmail);
      setName(userName);
    } catch (e) {
  ...
  const getThemeColor = () => {
    if (role === 'Teacher') return 'bg-purple-500 shadow-purple-500/20';
    if (role === 'Student') return 'bg-orange-500 shadow-orange-500/20';
      ...
      >
        <div className={`flex items-center h-20 border-b border-slate-800 transition-all ${isDesktopCollapsed ? 'justify-center px-0' : 'justify-between px-6'}`}>
          <div className="flex items-center gap-3">
            <div className={`${getThemeColor().split(' ')[0]} p-2 rounded-xl flex-shrink-0`}>
              <Settings className="w-6 h-6 text-white" />
            </div>
            {!isDesktopCollapsed && (
              <div className="overflow-hidden whitespace-nowrap transition-all opacity-100">
                <h1 className="text-xl font-bold tracking-tight">{role || 'System'}</h1>
                <p className="text-xs text-slate-400 font-medium">Assignment Management</p>
              </div>
            )}
          </div>
          {!isDesktopCollapsed && (
